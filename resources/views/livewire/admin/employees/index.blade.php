...
new class extends Component {
    ...
    public function loadSummaryStats(): void
    {
        ...
        $studentAttendance = \App\Models\Attendance::whereIn('employee_id', $studentIds)->whereDate('date', $today)->get();
        ...
        $this->notReportedCount = max(0, $this->totalStudents - $studentAttendance->whereIn('status', ['clocked_in', 'clocked_out'])->pluck('employee_id')->unique()->count());

        // Staff Stats
        $this->totalStaff = $allPeople->where('is_student', 0)->count();
        $staffIds = $allPeople->where('is_student', 0)->pluck('id');
        $staffAttendance = \App\Models\Attendance::whereIn('employee_id', $staffIds)->whereDate('date', $today)->get();
        ...
        $this->staffNotReportedCount = max(0, $this->totalStaff - $staffAttendance->whereIn('status', ['clocked_in', 'clocked_out'])->pluck('employee_id')->unique()->count());
    }
...
@push('styles')
    <style>
        ...
        @media (max-width: 768px) { .import-steps { grid-template-columns: 1fr; } }
        .import-step-card {
            background: #fff;
        ...
        .import-step-title { font-weight: 700; font-size: 0.85rem; color: #1e293b; margin-bottom: 0.25rem; }
        .import-step-desc  { font-size: 0.76rem; color: #64748b; margin: 0; }
        .import-step-tags  { display: flex; flex-wrap: wrap; gap: 5px; margin-top: 0.5rem; }
        .import-step-tag   {
        ...
        .import-dropzone-icon { font-size: 2.2rem; color: #cbd5e1; margin-bottom: 0.6rem; }
        .import-dropzone-text { font-size: 0.875rem; color: #64748b; margin: 0; }
        .import-dropzone-text a { color: #e14326; font-weight: 600; text-decoration: none; }
        .import-dropzone-hint { font-size: 0.72rem; color: #94a3b8; margin: 0.2rem 0 0; }
        .import-file-chosen {
            margin-top: 0.75rem;
            display: inline-flex; align-items: center; gap: 6px;
            background: #fde8e3; border-radius: 7px;
            padding: 5px 12px; font-size: 0.8rem; font-weight: 600; color: #c0341b;
        }

        /* Preview table */
        .import-preview-wrap { overflow-x: auto; border-radius: 10px; border: 1px solid #f0e4e0; }
        .import-preview-wrap table { margin: 0; font-size: 0.78rem; }
        .import-preview-wrap thead th { background: #fff5f2; color: #7c3022; font-weight: 700; font-size: 0.72rem; text-transform: uppercase; letter-spacing: 0.5px; border-bottom: 1.5px solid #f0ddd8; padding: 8px 12px; }
        .import-preview-wrap tbody td { padding: 7px 12px; vertical-align: middle; }
        .import-preview-wrap tbody tr:hover { background: #fffaf8; }
        ...
        .import-result-badge.success { background: #dcfce7; color: #15803d; }
        .import-result-badge.error   { background: #fee2e2; color: #dc2626; }
        .import-result-badge.zk-ok   { background: #e0f2fe; color: #0369a1; }
        .import-result-badge.zk-skip { background: #f1f5f9; color: #64748b; }
        .import-result-badge.zk-err  { background: #fef9c3; color: #a16207; }
        ...
        .summary-card { background: #fff; border: 1px solid rgba(0,0,0,0.06); border-radius: 14px; padding: 1.4rem 1.5rem 1.2rem; height: 100%; transition: transform 0.25s ease, box-shadow 0.25s ease; }
        .summary-card:hover { transform: translateY(-3px); box-shadow: 0 8px 20px rgba(0,0,0,0.09); }
        .summary-card-icon { width: 42px; height: 42px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 22px; margin-bottom: 0.9rem; }
        .summary-card-title { font-size: 0.72rem; font-weight: 600; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.6px; margin-bottom: 0.35rem; }
        .summary-card-value { font-size: 2rem; font-weight: 700; line-height: 1; color: #1e293b; margin-bottom: 0.3rem; }
        .summary-card-subtitle { font-size: 0.8rem; color: #64748b; margin: 0; }
        .summary-card-bar { height: 4px; border-radius: 99px; background: #f1f5f9; margin-top: 0.85rem; overflow: hidden; }
        .summary-card-bar-fill { height: 100%; border-radius: 99px; transition: width 0.6s ease; }
        .summary-stats-row { margin-bottom: 2rem; }
        .person-type-toggle { display: flex; border: 1px solid #d1d5db; border-radius: 8px; overflow: hidden; }
        .person-type-toggle input[type="radio"] { display: none; }
        .person-type-toggle label { flex: 1; text-align: center; padding: 9px 16px; font-size: 0.875rem; font-weight: 600; cursor: pointer; background: #f8fafc; color: #64748b; margin: 0; transition: all 0.2s; }
        .person-type-toggle input[type="radio"]:checked + label { background: var(--primary-color) !important;; color: white; }
        .type-tab { display: inline-flex; border: 1px solid #d1d5db; border-radius: 8px; overflow: hidden; }
        .type-tab button { padding: 7px 20px; font-size: 0.875rem; font-weight: 600; border: none; background: #f8fafc; color: #64748b; cursor: pointer; transition: all 0.2s; }
        .type-tab button.active { background: var(--primary-color) !important; color: white; }
        .type-tab button:not(:last-child) { border-right: 1px solid #d1d5db; }
        .form-control { display: block !important; font-size: 0.875rem !important; line-height: 1.5 !important; color: #1e293b !important; background-color: #fff !important; border: 1px solid #d1d5db !important; border-radius: 8px !important; transition: all 0.2s ease-in-out !important; }
        table.dataTable td { vertical-align: middle !important; }
        iconify-icon { vertical-align: middle !important; }
        .btn-group > div > button.dropdown-toggle { background-color: #f4f4f5; border: 1px solid #cbd5e1; color: #1e293b; padding: 8px 12px; border-radius: 8px; font-weight: 500; font-size: 0.875rem; display: inline-flex; align-items: center; gap: 8px; margin-left: 5px; }
        .btn-group > div > button.dropdown-toggle:hover { background-color: #e2e8f0; }
        .btn-group > .dropdown-menu { position: fixed !important; top: 100px !important; left: 50% !important; transform: translateX(-50%) !important; width: 600px !important; max-width: 90vw !important; padding: 24px !important; border-radius: 16px; background-color: #ffffff; box-shadow: 0 12px 30px rgba(0,0,0,0.15); border: 1px solid #e5e7eb; z-index: 1050; overflow-y: auto; max-height: 70vh; }
        .filter-close-button { position: absolute; top: 12px; right: 25px; background: transparent; border: none; font-size: 1.7rem; color: #6b7280; cursor: pointer; z-index: 1100; }
        .filter-close-button:hover { color: #ef4444; }
        #table-bulkActionsDropdown { background-color: var(--primary-color) !important; border: none; color: #fff; font-weight: 600; }
        #table-bulkActionsDropdown:hover { background-color: var(--primary-color) !important; }
    </style>
@endpush
...
<div class="row">
    <div class="col-12">
        ...
        <div class="row g-3 mb-4 summary-stats-row">
            @php
                // Determine which set of data to show based on the toggle
            ...
            <div class="col-lg-3 col-md-6 col-12">
                <div class="summary-card">
                    ...
                    <div class="summary-card-bar">
                        <div class="summary-card-bar-fill" style="width:{{ $total > 0 ? ($present/$total)*100 : 0 }}%; background:#22c55e;"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-6 col-12">
                <div class="summary-card">
                    ...
                    <div class="summary-card-bar">
                        <div class="summary-card-bar-fill" style="width:{{ $total > 0 ? ($left/$total)*100 : 0 }}%; background:#0ea5e9;"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-6 col-12">
                <div class="summary-card">
                    ...
                    <div class="summary-card-bar">
                        <div class="summary-card-bar-fill" style="width:{{ $total > 0 ? ($missing/$total)*100 : 0 }}%; background:#ef4444;"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card card-body">
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center mb-3 gap-2">
                ...
                @if($isStudentOrg)
                    <div class="type-tab">
                        <button wire:click="switchType('student')" class="{{ $personType === 'student' ? 'active' : '' }}">
                            <iconify-icon icon="mdi:school" style="font-size:15px;margin-right:4px;"></iconify-icon>Students
                        </button>
                        <button wire:click="switchType('staff')" class="{{ $personType === 'staff' ? 'active' : '' }}">
                            <iconify-icon icon="mdi:account-tie" style="font-size:15px;margin-right:4px;"></iconify-icon>Staff
                        </button>
                    </div>
                @else
                    <div></div>
                ...
                <div class="d-flex gap-2">
                    ...
                    >
                        <iconify-icon icon="{{ $showImportPanel ? 'mdi:close' : 'mdi:upload' }}" style="font-size:17px;"></iconify-icon>
            ...
            @if($showImportPanel)
                <div class="import-accordion-wrap mb-3">
                    ...
                    <div class="import-header" wire:click="toggleImportPanel">
                        <div class="import-header-left">
                            ...
                            <div>
                                ...
                                <p class="import-header-sub">Download the template, fill in data, upload to create multiple records + ZKBio sync</p>
                            </div>
                        </div>
                        ...
                        @if(!$importParsed && !$importProcessed)
                            <div class="import-steps">
                                ...
                                <div class="import-step-card">
                                    ...
                                    <p class="import-step-desc">Get the CSV template with the correct columns pre-configured.</p>
                                    ...
                                    <div class="mt-3 d-flex gap-2">
                                        <button wire:click="downloadTemplate('csv')" type="button"
                                            ...
                                            <iconify-icon icon="mdi:file-delimited-outline" style="margin-right:4px;"></iconify-icon>
                                            Download .csv
                                        </button>
                                        <button wire:click="downloadTemplate('xlsx')" type="button"
                                            ...
                                            <iconify-icon icon="mdi:microsoft-excel" style="margin-right:4px;"></iconify-icon>
                                            Download .xlsx
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div class="import-step-card">
                                    ...
                                    <p class="import-step-desc mt-2" style="color:#e14326; font-weight:600;">
                                        <iconify-icon icon="mdi:information-outline" style="margin-right:3px;"></iconify-icon>
                                ...
                                <div class="import-step-card">
                                    ...
                                    <p class="import-step-desc">Upload your file. Review the preview, confirm to create all records and sync to <strong>ZKBio</strong>.</p>
                    ...
                    <span class="import-step-tag" style="background:#e0f2fe;color:#0369a1;">
                        ...
                        {{-- ── Error alert ──────────────────────────────────────────────── --}}
                        @if($importError)
                            <div class="alert alert-danger d-flex align-items-center gap-2 py-2 px-3 mb-3" style="border-radius:9px; font-size:0.82rem;">
                                <iconify-icon icon="mdi:alert-circle-outline" style="font-size:18px;flex-shrink:0;"></iconify-icon>
                                {{ $importError }}
                            </div>
                        @endif
                        ...
                        @if(!$importParsed && !$importProcessed)
                            <form wire:submit.prevent="parseImportFile">
                                ...
                                @error('importFile') <small class="text-danger d-block mt-1">{{ $message }}</small> @enderror
                                ...
                                <div class="d-flex justify-content-end mt-3">
                                    <button type="submit" class="btn btn-primary d-flex align-items-center gap-2"
                                        ...
                                        <iconify-icon icon="mdi:table-eye" wire:loading.remove wire:target="parseImportFile"></iconify-icon>
                                        Preview Data
                                    </button>
                                </div>
                            </form>
                        ...
                        @if($importParsed && !$importProcessed)
                            <div class="d-flex align-items-center justify-content-between mb-2 flex-wrap gap-2">
                                <p class="mb-0" style="font-size:0.82rem; font-weight:600; color:#1e293b;">
                                    <iconify-icon icon="mdi:table-check" style="color:#e14326;margin-right:4px;"></iconify-icon>
                                    Preview — <span style="color:#e14326;">{{ count($importPreview) }} rows</span> ready to import
                                </p>
                                <button wire:click="resetImport" type="button"
                                    ...
                                    <iconify-icon icon="mdi:refresh"></iconify-icon> Change File
                                </button>
                            </div>
                            ...
                            <div class="import-preview-wrap mb-3">
                                <table class="table table-hover table-bordered mb-0">
                                    ...
                                    @if(count($importPreview) > 20)
                                        <tr>
                                            <td colspan="99" class="text-center text-muted py-2" style="font-size:0.75rem;">
                            ...
                            <div class="alert d-flex align-items-start gap-2 py-2 px-3 mb-3"
                                ...
                                <iconify-icon icon="mdi:fingerprint" style="font-size:18px;flex-shrink:0;margin-top:1px;"></iconify-icon>
                                <span>
                    ...
                    If ZKBio sync fails for a row, the record is still saved — you can re-sync manually.
                            ...
                            <div class="d-flex justify-content-end gap-2">
                                ...
                                <button wire:click="commitImport" type="button"
                                    ...
                                    <iconify-icon icon="mdi:check-bold" wire:loading.remove wire:target="commitImport"></iconify-icon>
                        ...
                        @if($importProcessed)
                            ...
                            <div class="import-summary-bar">
                                <iconify-icon icon="mdi:check-circle" style="font-size:22px; color:#16a34a;"></iconify-icon>
                    ...
                    ZKBio sync attempted for all created records
                                ...
                                <div class="ms-auto">
                                    <button wire:click="resetImport" type="button"
                                        ...
                                        <iconify-icon icon="mdi:upload"></iconify-icon> Import Another File
                                    </button>
                                </div>
                            </div>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            @if($isStudentOrg)
                                <div class="col-12 mb-3">
                                    ...
                                    <div class="person-type-toggle">
                                        <input type="radio" wire:model.live="personType" value="student" id="typeStudent">
                                        <label for="typeStudent"><iconify-icon icon="mdi:school" style="font-size:16px;margin-right:4px;"></iconify-icon>Student</label>
                                        <input type="radio" wire:model.live="personType" value="staff" id="typeStaff">
                                        <label for="typeStaff"><iconify-icon icon="mdi:account-tie" style="font-size:16px;margin-right:4px;"></iconify-icon>Staff</label>
                                    </div>
                                </div>
                            @endif
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" wire:model="name" class="form-control" placeholder="{{ $isStudent ? 'e.g. Jane Kamau' : 'e.g. Mr. James Odhiambo' }}"/>
                                @error('name') <small class="text-danger">{{ $message }}</small> @enderror
                            </div>

                            <div class="col-md-6 mb-3">
                                <label class="form-label">{{ $isStudent ? 'Student ID / Admission No.' : 'ID Number' }} <span class="text-danger">*</span></label>
                                <input type="text" wire:model="id_number" class="form-control" placeholder="{{ $isStudent ? 'e.g. STU-0041' : 'e.g. 12345678' }}"/>
                            ...
                            @if(!$isStudentOrg || $personType === 'staff')
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="email" wire:model="email" class="form-control" placeholder="e.g. james.o@school.ac.ke"/>
                                    @error('email') <small class="text-danger">{{ $message }}</small> @enderror
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label class="form-label">Phone Number <span class="text-danger">*</span></label>
                                    <input type="text" wire:model="phone" class="form-control" placeholder="e.g. 254712345678"/>
...
{{--                            </div>--}}
                            ...
                            <div class="col-md-6 mb-3">
                                <label class="form-label">{{ $isStudent ? 'Stream (optional)' : 'Title / Role' }}</label>
                                <input type="text" wire:model="employee_title" class="form-control" placeholder="{{ $isStudent ? 'e.g. North, East, Blue' : 'e.g. Teacher, Nurse, Housemaster' }}"/>
                ...
                <form wire:submit.prevent="assignWorkLocation">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" wire:keyup.debounce.500ms="$dispatch('search-work-location')" wire:model="search" class="form-control" placeholder="Type to search locations..."/>
                            @if(!empty($search) && !$selectedLocation)
                                <ul class="list-group mt-2" style="max-height:200px;overflow-y:auto;">
                                    @forelse($workLocations as $location)
                                        <li class="list-group-item list-group-item-action" wire:click="selectWorkLocation({{ $location->id }})" style="cursor:pointer;">
            ...
            <div class="modal-content shadow-lg">
                ...
                <div class="modal-body">
                    <div class="alert {{ $shiftStatus === 'off_shift' ? 'alert-warning' : 'alert-success' }}">
                        Current status: <strong>{{ $shiftStatus === 'off_shift' ? 'Off Shift' : 'On Active Shift' }}</strong>
                    </div>
                    <div class="mb-3">
...
@push('scripts')
    <script>
        window.addEventListener('show-work-location-modal', () => { new bootstrap.Modal(document.getElementById('workLocationModal')).show(); });
        window.addEventListener('hide-work-location-modal', () => { bootstrap.Modal.getInstance(document.getElementById('workLocationModal'))?.hide(); });
        window.addEventListener('show-employee-modal',      () => { new bootstrap.Modal(document.getElementById('employeeModal')).show(); });
        window.addEventListener('hide-employee-modal',      () => { bootstrap.Modal.getInstance(document.getElementById('employeeModal'))?.hide(); });
        window.addEventListener('show-off-shift-modal',     () => { new bootstrap.Modal(document.getElementById('offShiftModal')).show(); });
        window.addEventListener('hide-off-shift-modal',     () => { bootstrap.Modal.getInstance(document.getElementById('offShiftModal'))?.hide(); });
        ...
        document.addEventListener('DOMContentLoaded', () => {
            const observer = new MutationObserver(() => {
                ...
                if (dropdown && !dropdown.querySelector('.filter-close-button')) {
                    ...
                    closeBtn.onclick = () => { document.querySelector('.dropdown-menu[role="menu"]')?.classList.remove('show'); };
