...
new class extends Component {
    ...
    public function openAddJobTitleModal(): void
    {
        ...
        $hierarchyService = app(\App\Services\OrganizationHierarchyService::class);

        $hierarchy = $hierarchyService->build($this->organizationId);


        Log::info('Organization Hierarchy Tree: ' . json_encode($hierarchy));
    ...
    public function saveJobTitle(): void
    {
        ...
        // check if same job title name exists for the same organization
    ...
    {
        // $this->jobTitlesQuery()->whereKey($id)->delete();

        // if ($this->getPaginatedJobTitlesProperty()->isEmpty() && $this->getPage() > 1) {
        //     $this->previousPage();
        // }
    ...
    public function titleFilter(string $role): void
    {
    ...
    public function getPaginatedJobTitlesProperty()
    {
        $search = trim($this->search);

        return $this->jobTitlesQuery()
            ->when($search !== '', function (Builder $query) use ($search) {
                $query->where(function (Builder $subQuery) use ($search) {
                    $subQuery->where('name', 'like', "%{$search}%");
                });
            })
            ->when($this->titleFilter !== 'All', function (Builder $query) {
                $query->where('role', $this->titleFilter);
            })
            ->latest('id')
            ->paginate(10);
    }
    ...
    {
        return $this->jobTitlesQuery()->count();
    }
};
?>
...
<div class="sj-users-shell">
            ...
            <div class="toolbar-right">
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end sj-filter-menu">
                        <li><button type="button" class="dropdown-item {{ $titleFilter === 'All' ? 'active' : '' }}" wire:click="titleFilter('All')">All roles</button></li>
                        <li><button type="button" class="dropdown-item {{ $titleFilter === 'Admin' ? 'active' : '' }}" wire:click="titleFilter('Admin')">Admin</button></li>
                        <li><button type="button" class="dropdown-item {{ $titleFilter === 'Employee' ? 'active' : '' }}" wire:click="titleFilter('Employee')">Employee</button></li>
                        <li><button type="button" class="dropdown-item {{ $titleFilter === 'Supervisor' ? 'active' : '' }}" wire:click="titleFilter('Supervisor')">Supervisor</button></li>
                    </ul>
                </div>
        ...
        <table class="p-0">
            ...
            <tbody>
                @forelse ($this->paginatedJobTitles as $jobTitle)
                    <tr>
                        <td>
                            <div class="user-cell">
                        ...
                        <td><span class="role-pill">{{ $jobTitle->createdBy->name ?? 'N/A' }}</span></td>
                        <td><span class="role-pill">LEVELS</span></td>
                            ...
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end sj-action-menu">
                                    ...
                                    <li class="menu-item p-2">
                                        ...
                                        @endif
                                        <button type="button" class="menu-item" wire:click="toggleJobTitleAccess({{ $jobTitle->id }})">{{ $jobTitle->is_active ? 'Deactivate Job Title' : 'Reactivate job title' }}</button>
                                    </li>
                ...
                @empty
                    <tr>
                        <td colspan="5" style="text-align: center; color: var(--ink-3);">No job titles match your search.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="footer-row">
            <span>
                @if ($this->paginatedJobTitles->total() > 0)
                    Showing {{ $this->paginatedJobTitles->firstItem() }} to {{ $this->paginatedJobTitles->lastItem() }} of {{ $this->paginatedJobTitles->total() }} job titles
                @else
                    Showing 0 job titles
                @endif
            </span>
            <div class="page-btns">
                <button class="page-btn" type="button" wire:click="previousPage" @disabled($this->paginatedJobTitles->onFirstPage())>&lsaquo;</button>
                <button class="page-btn current" type="button">{{ $this->paginatedJobTitles->currentPage() }}</button>
                <button class="page-btn" type="button" wire:click="nextPage" @disabled(! $this->paginatedJobTitles->hasMorePages())>&rsaquo;</button>
            </div>
        </div>
