...
class AutoDeactivateEmployees extends Command
{
    ...
    public function handle(): int
    {
        ...
        if ($orgIds->isEmpty()) {
            $this->info('No organizations have the auto-deactivation policy enabled.');
        ...
        foreach ($orgIds as $orgId) {
            ...
            if ($value <= 0) {
                $this->warn("Org {$orgId} ({$org->name}): policy enabled but no valid duration set — skipping.");
                continue;
            }
            ...
            foreach ($employees as $employee) {
                // ->save() (not a mass update) so Employee::saving() stamps deactivated_at.
                $employee->active = false;
                $employee->save();
                $totalDeactivated++;
            }

            $this->info("Org {$orgId} ({$org->name}): deactivated {$employees->count()} employee(s) created before {$cutoff->toDateString()}.");
        }

        $this->info("Done. Total deactivated: {$totalDeactivated}.");
