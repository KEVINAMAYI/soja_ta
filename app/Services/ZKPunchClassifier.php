...
class ZKPunchClassifier
{
    ...
    private function classifyWeekendOt(array $filtered, object $shift, string $today, bool $isSaturday, array $result): array
    {
        ...
        $checkIn = $filtered[0];
        $checkOut = end($filtered);

        // Saturday: cap OT1 at 4:30pm per instructions
        if ($isSaturday) {
            $ot1Cap = Carbon::parse($today . ' 16:30:00');
            $effectiveOut = $checkOut->gt($ot1Cap) ? $ot1Cap : $checkOut;
            $workedMinutes = (int)$checkIn->diffInMinutes($effectiveOut);
            $result['ot1_hours'] = round(max(0, $workedMinutes) / 60, 2);
            $result['check_out'] = $effectiveOut; // cap the checkout too
            $result['notes'][] = "Saturday OT1 capped at 16:30 — {$result['ot1_hours']}h";
        } else {
            // Sunday: all hours are OT2, no cap
            $workedMinutes = (int)$checkIn->diffInMinutes($checkOut);
            $result['ot2_hours'] = round(max(0, $workedMinutes) / 60, 2);
            $result['notes'][] = "Sunday OT2 — {$result['ot2_hours']}h";
        }

        $result['worked_hours'] = $isSaturday ? $result['ot1_hours'] : $result['ot2_hours'];
