 ...
 */
class AttendanceReportHttpService
{
    // =========================================================================
    ...
    )
    {
        ...
        if ($status) {
            $records = $records->unique(fn($item) => $item->employee_id . '_' . $item->date)->values();
        }

        return $this->paginate($records, $page, $perPage);
    }

    public function baseQuery(int $orgId, array $ids = [], ?string $start_date = null, ?string $end_date = null, array $department_ids = [])
    {
    ...
    public function getCustomAttendanceSummary(int $orgId, array $ids, string $start_date, string $end_date, ?array $department_ids = [], ?int $page = null, ?int $perPage = null)
    {
    ...
    public function getByDepartment($orgId, $ids, $start_date, $end_date, ?int $page = null, ?int $perPage = null)
    {
    ...
    )
    {
        ...
        $decorated = $interpreter->decorateCollection($records);

        return $this->paginate($decorated, $page, $perPage);
    }
    ...
    )
    {
        ...
        $results = $byEmployee->map(function ($employeeRows) {
        ...
        return $this->paginate($results, $page, $perPage);
    }
    ...
    )
    {
        ...
        $results = $rows->groupBy('department_id')->map(function ($deptRows) {
    ...
    /**
     * Paginate an already-fetched collection/array. When $perPage is null, the
     * collection is returned unchanged (keeps existing non-paginated callers working).
     */
    private function paginate(iterable $records, ?int $page = null, ?int $perPage = null): iterable|LengthAwarePaginator
    {
        if (!$perPage) {
            return $records;
        }
